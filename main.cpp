...
//test code
constexpr int BUFFER_SIZET = 10;
char buffer[BUFFER_SIZET];

int main(){
    USBSerial usbserial{};
    usbserial.init();
    while(true){
        int readAmount = usbserial.read(&buffer, BUFFER_SIZET);
        if(readAmount > 0) usbserial.write(&buffer, readAmount);
    }
}
